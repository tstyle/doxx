...
def pull_textfile_runner(text_url_dict):
    ...
    if number_of_files > 0:   # multiple text file pull, each in separate process
        if number_of_files > 1:
            processes = []       # list of spawned processes
            outputlock = Lock()  # stdout / stderr writes lock
            
            for file_path in file_list:
                p = Process(target=_pull_textfile_multiprocess, args=(file_path, text_url_dict[file_path], outputlock))
                p.start()
                processes.append(p)
...
def _pull_textfile_multiprocess(file_path, url, outputlock):
    # create OS dependent file path (if necessary)
    file_path = _make_os_dependent_path(file_path)
    # make directory structure if necessary for the file path
    _create_pull_dirs(file_path)
...
def _pull_github_repo_multiprocess(file_path, url, outputlock):
    pass
